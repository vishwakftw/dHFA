...
dhfaeg_avg = algos.DHFA(f=f, eta=1 / L**0.5, lam=lam, integrator="extg", aggfunc="avg")
x_dfhaeg_avg_sol, f_vals_dhfaeg_avg = dhfaeg_avg.run(x_0=x_0, K=K, N=N)

dhfaeg_savg = algos.DHFA(
    f=f, eta=1 / L**0.5, lam=lam, integrator="extg", aggfunc="s-avg"
)
x_dfhaeg_savg_sol, f_vals_dhfaeg_savg = dhfaeg_savg.run(x_0=x_0, K=K, N=N)
...
if args.expt_type == "lin-reg":
    ...
    min_val = min(
        min(f_vals_dhfaeg_avg),
        min(f_vals_dhfaeg_savg),
        min(f_vals_gd),
        min(f_vals_agd),
    )
    ylims = {"bottom": 1e-5}
...
ax.plot(
    ...
    (f_vals_dhfaeg_avg - min_val).clamp_min_(torch.tensor(0.0)),
    label="dHFA-eg (avg)",
    marker="o",
    lw=3.5,
    markersize=10,
...
ax.plot(
    [0] + np.cumsum(N).tolist(),
    (f_vals_dhfaeg_savg - min_val).clamp_min_(torch.tensor(0.0)),
    label="dHFA-eg (s-avg)",
    marker="*",
    lw=3.5,
    markersize=10,
