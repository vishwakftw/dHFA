...
momentum = None
if args.beta > 0:
    momentum = (1 - (args.beta * best_eta_agd) ** 0.5) / (
        1 + (args.beta * best_eta_agd) ** 0.5
    )
...
N, K = algos.make_NK_for_dhfa(
    eta=best_eta_dhfaeg, total_iters=args.total_iters, alpha=args.beta, lam=lam
)
...
plt.tight_layout()
plt.savefig(
    f"logistic-regression-beta={args.beta},N={args.N},d={args.d},seed={args.seed}.pdf"
)
